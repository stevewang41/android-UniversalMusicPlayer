...
public final class LocalPlayback implements Playback {
    ...
    /** 与 PlaybackManager 交互的接口 */
    private Callback mCallback;
    /** 歌曲数据提供者 */
    private final MusicProvider mMusicProvider;
    /** Wifi锁，用来锁定Wifi网络，使其一直保持连接 */
    private final WifiManager.WifiLock mWifiLock;
    /** 音频管理器，负责管理音频焦点 */
    private final AudioManager mAudioManager;
    /** 当前音频焦点的状态 */
    private int mAudioFocusState = AUDIO_NO_FOCUS_NO_DUCK;
    /** 再次获取到音频焦点时是否继续播放 */
    ...
    /** 当播放器实例为null时，播放状态是否返回 {@link PlaybackStateCompat#STATE_STOPPED} */
    private boolean mExoPlayerNullIsStopped = false;
    /** 播放器事件监听 */
    private final ExoPlayerEventListener mPlayerEventListener = new ExoPlayerEventListener();
    ...
    public LocalPlayback(Context context, MusicProvider musicProvider) {
        ...
        mWifiLock = ((WifiManager) mAppContext.getSystemService(Context.WIFI_SERVICE))
    ...
    public void play(QueueItem item) {
        ...
        if (mediaHasChanged || mExoPlayer == null) {
            ...
            DataSource.Factory dataSourceFactory = new DefaultDataSourceFactory(
                            mAppContext, Util.getUserAgent(mAppContext, "uamp"), null);
    ...
    /**
     * 根据当前音频焦点的状态，重新配置播放器 以及 启动/重新启动 播放器
     * 如果我们持有音频焦点，则正常播放音频；如果我们失去音频焦点，播放器将暂停播放或者设置为低音量，这取决于当前焦点设置允许哪种设置
     */
    private void configurePlayerState() {
        LogHelper.d(TAG, "configurePlayerState. mCurrentAudioFocusState=", mAudioFocusState);
    ...
    private void registerAudioNoisyReceiver() {
        if (!mAudioNoisyReceiverRegistered) {
            mAppContext.registerReceiver(mAudioNoisyReceiver, mAudioNoisyIntentFilter);
    ...
    private final BroadcastReceiver mAudioNoisyReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (AudioManager.ACTION_AUDIO_BECOMING_NOISY.equals(intent.getAction())) {
                ...
                // 当音乐正在播放中，通知MusicService暂停播放音乐（在MusicService.onStartCommand中处理此命令）
                if (isPlaying()) {
    ...
    private final class ExoPlayerEventListener implements Player.EventListener {
        ...
        public void onPlayerStateChanged(boolean playWhenReady, int playbackState) {
            switch (playbackState) {
                ...
                case Player.STATE_READY:
                    if (mCallback != null) {
                        mCallback.onPlaybackStateChanged(getState());
                    }
                    break;
                case Player.STATE_ENDED:
